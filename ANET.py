...
class ANET(nn.Module):
    ...
    def get_layers(self, hidden_layers, activation_function):
        ...
        layers = [torch.nn.Linear(
            self.input_size, hidden_layers[0])]  # arg: input layer size, output layer size
        # During training, randomly zeroes some of the elements of the input tensor
        layers.append(torch.nn.Dropout(p=0.2))
        layers.append(
            activation_function) if activation_function != None else None
        for i in range(len(hidden_layers)-1):
            # arg: input layer size, output layer size
